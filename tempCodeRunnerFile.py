...
class NeuralNet:
    ...
    def mutate(self, rate=BASE_MUTATION_RATE):
        with torch.no_grad():
            self.w1 += torch.randn_like(self.w1) * rate * (torch.rand_like(self.w1) < 0.4)
            self.w2 += torch.randn_like(self.w2) * rate * (torch.rand_like(self.w2) < 0.4)
            self.b1 += torch.randn_like(self.b1) * rate * (torch.rand_like(self.b1) < 0.4)
            self.b2 += torch.randn_like(self.b2) * rate * (torch.rand_like(self.b2) < 0.4)
...
class Car:
    ...
    def apply_outputs(self, outputs, dt):
        ...
        # Goal-directed fitness: speed MULTIPLIES progress (fast corners = huge reward)
        ...
        # Speed multiplier: crawling at 10% speed gets 0.4x, full speed gets 2.0x
        speed_mult = 0.3 + speed_ratio * 1.7
        if progress > 0:
            self.fitness += progress * 2.0 * speed_mult
        else:
            self.fitness += progress * 0.5

        # Continuous speed bonus - strongly reward sustained speed
        self.fitness += speed_ratio * speed_ratio * 0.8 * dt * 60

        # Harsh penalty for crawling (under 40% max speed)
        if self.speed < MAX_SPEED * 0.4:
            self.fitness -= 0.5 * dt * 60
        
        # Idle check - give cars time to recover
        if self.speed < 2:
            self.idle_time += dt
            if self.idle_time > IDLE_TIMEOUT:
                self.alive = False
        else:
            self.idle_time = max(0, self.idle_time - dt * 2)  # Gradual recovery
        ...
        # Checkpoint - progressive reward (later CPs worth more to push past crash frontier)
        cp_x, cp_y = self.track.cp_centers[self.next_cp]
        if math.hypot(self.x - cp_x, self.y - cp_y) < 30:
            cp_bonus = 150.0 + self.next_cp * 5.0
            self.next_cp = (self.next_cp + 1) % CHECKPOINT_COUNT
            self.fitness += cp_bonus
            if self.next_cp == 0:
                self.laps += 1
                self.fitness += 1000.0
...
def evolve(cars, track, stagnation_count=0):
    ...
    is_stagnant = stagnation_count >= STAGNATION_GENS

    new_cars = []
    # More elites during stagnation (they reliably reach the crash frontier)
    elite_count = 8 if is_stagnant else 5
    for i in range(min(elite_count, len(survivors))):
        new_cars.append(Car(track, survivors[i].brain.copy(), len(new_cars)))

    if is_stagnant:
        # STAGNATION: mutation spectrum - tiny to wild
        # 30% tiny mutations: reliably reach crash point, slight variation there
        for _ in range(POPULATION * 30 // 100):
            if len(new_cars) >= POPULATION: break
            brain = best_brain.copy()
            brain.mutate(BASE_MUTATION_RATE * 0.15)
            new_cars.append(Car(track, brain, len(new_cars)))

        # 20% medium mutations: moderate exploration around crash point
        for _ in range(POPULATION * 20 // 100):
            if len(new_cars) >= POPULATION: break
            brain = best_brain.copy()
            brain.mutate(BASE_MUTATION_RATE * 0.7)
            new_cars.append(Car(track, brain, len(new_cars)))

        # 15% wild mutations: might find a completely different approach
        for _ in range(POPULATION * 15 // 100):
            if len(new_cars) >= POPULATION: break
            brain = best_brain.copy()
            brain.mutate(BASE_MUTATION_RATE * 2.5)
            new_cars.append(Car(track, brain, len(new_cars)))

        # Rest from top 5 survivors
        while len(new_cars) < POPULATION:
            parent = random.choice(survivors[:5])
            brain = parent.brain.copy()
            brain.mutate(BASE_MUTATION_RATE * 0.4)
            new_cars.append(Car(track, brain, len(new_cars)))
    else:
        # NORMAL: low mutations so children reliably replicate parent behavior
        # 45% tiny mutations of best - reach same checkpoint, vary at the frontier
        for _ in range(POPULATION * 45 // 100):
            if len(new_cars) >= POPULATION: break
            brain = best_brain.copy()
            brain.mutate(BASE_MUTATION_RATE * 0.25)
            new_cars.append(Car(track, brain, len(new_cars)))

        # 20% small mutations of second best
        for _ in range(POPULATION * 20 // 100):
            if len(new_cars) >= POPULATION: break
            brain = second_best.copy()
            brain.mutate(BASE_MUTATION_RATE * 0.35)
            new_cars.append(Car(track, brain, len(new_cars)))

        # Rest from top survivors with moderate mutations
        while len(new_cars) < POPULATION:
            weights = [1.0 / (i + 1) for i in range(len(survivors))]
            parent = random.choices(survivors, weights=weights, k=1)[0]
            brain = parent.brain.copy()
            brain.mutate(BASE_MUTATION_RATE * 0.5)
            new_cars.append(Car(track, brain, len(new_cars)))
...
def main():
    ...
    while True:
        ...
        # Batched inference using persistent BatchedBrains (no per-frame allocation)
        alive_mask[:] = False
        alive_count = 0
        for c in cars:
            if c.alive:
                all_inputs[c.idx] = c.get_inputs()
